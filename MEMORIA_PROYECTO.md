...
* **Detección de "No cruzaron" (anti-duplicados por typo/variante):** Cuando un código del origen NO cruza exacto con ningún SKU de la maestra, antes de crearlo como fila nueva el motor verifica si se *parece* a un SKU existente (`find_similar_sku` en `services.py`). Si hay parecido, NO se crea (para evitar duplicados) y se marca como **sospechoso** para revisión manual en la cola de Staging (resaltado ámbar). Motivos: `formato` (`1203.0`/`01203`/mayúsculas), `variante` (`1203-1` cuando existe `1203`) y `similar` (fuzzy ≥0.86 para typos). La comparación es SOLO para detectar; el valor real del SKU nunca se altera. Los códigos que no se parecen a nada siguen entrando como nuevos legítimos.
